  ...
  const handleClearCache = () => {
    if (window.confirm('Are you sure you want to clear all local data? This will reset all products, categories, deals, slides, and settings to their defaults. This action cannot be undone.')) {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('katariya-auto-parts-')) {
    ...
    <div className="max-w-4xl animate-fade-in">
      <ScrollReveal>
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            onClick={handleSave}
            className="flex items-center gap-2 bg-[#dc2626] hover:bg-[#b91c1c] text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {[
            ...
            <Link key={item.path} to={item.path}
              className="flex items-center gap-2 bg-red-50 hover:bg-red-100 border border-red-100 text-red-700 font-semibold px-4 py-3 rounded-xl transition-colors text-sm">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-4 font-semibold text-sm transition-all relative whitespace-nowrap ${
                    activeTab === tab.id ? 'text-[#dc2626]' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <Icon size={16} /> {tab.label}
                  {activeTab === tab.id && (
                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#dc2626] rounded-t-full" />
                  )}
                </button>
              );
            })}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="text" name="storeName" value={formData.storeName || ''} onChange={handleChange}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#dc2626] focus:ring-2 focus:ring-red-100 transition-all font-medium"
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <input type="file" accept="image/*" onChange={handleLogoUpload} disabled={isUploading}
                        className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-[#dc2626] hover:file:bg-red-100 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" />
                ...
                <div>
                  ...
                  <input type="text" name="logoChar" value={formData.logoChar || ''} onChange={handleChange}
                    maxLength={1}
                    className="w-20 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#dc2626] focus:ring-2 focus:ring-red-100 transition-all font-black text-xl text-center"
                    placeholder="P" />
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <textarea name="footerDescription" value={formData.footerDescription || ''} onChange={handleChange}
                    rows={4}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#dc2626] focus:ring-2 focus:ring-red-100 transition-all font-medium resize-none"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="url" name="facebookUrl" value={formData.facebookUrl || ''} onChange={handleChange}
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#1877F2] focus:ring-2 focus:ring-red-100 transition-all font-medium"
