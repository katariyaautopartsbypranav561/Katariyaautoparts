...
function ToggleSwitch({ enabled, onChange, id }) {
  return (
    <button
      ...
      className={`relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#dc2626] focus:ring-offset-2 ${
...
// ─── Section Card ────────────────────────────────────────────────────────────
function SectionCard({ title, icon: Icon, iconColor = 'text-[#dc2626]', iconBg = 'bg-red-50', children, badge }) {
  return (
...
function InfoBanner({ icon: Icon, children, color = 'blue' }) {
  const colorMap = {
    blue:  { bg: 'bg-red-50',  border: 'border-red-100',  text: 'text-red-800',  icon: 'text-red-500'  },
    amber: { bg: 'bg-red-50', border: 'border-red-100', text: 'text-red-800', icon: 'text-red-500' },
    ...
    <div className="max-w-3xl animate-fade-in">
      ...
      <ScrollReveal>
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 bg-[#dc2626] hover:bg-[#b91c1c] disabled:opacity-70 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
            <Save size={18} />
      ...
      <div className="space-y-6">
        ...
        <ScrollReveal delay={50}>
          <div className="bg-gradient-to-r from-[#dc2626]/10 to-red-50 border border-red-200 rounded-2xl p-5 flex gap-4 items-start">
            <div className="p-3 bg-[#dc2626] rounded-xl shrink-0">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                ...
                reference ID. Your{' '}
                <span className="font-semibold text-[#dc2626]">Key ID</span> is safe to expose on
                the frontend; your{' '}
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 pr-12 bg-gray-50 border rounded-xl focus:outline-none focus:ring-2 transition-all font-mono text-sm ${
                          !keyIsValid
                            ...
                            : 'border-gray-200 focus:border-[#dc2626] focus:ring-red-100'
                        }`}
                      />
                      {razorpayKeyId && (
        ...
        {onlinePaymentEnabled && (
          <ScrollReveal delay={150}>
            <SectionCard
              title="Gateway Mode"
              icon={FlaskConical}
              iconColor={testMode ? 'text-red-500' : 'text-green-600'}
              iconBg={testMode ? 'bg-red-50' : 'bg-green-50'}
            >
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-2">
                  ...
                  >
                    ...
                    {testMode && (
                      <Radio size={16} className="ml-auto text-red-500 shrink-0" />
                    )}
                  </label>
                ...
                {!testMode && (
                  <InfoBanner icon={AlertTriangle} color="amber">
                    ...
                    <code className="bg-red-100 px-1 rounded text-xs">rzp_live_</code>.
                  </InfoBanner>
                )}
              </div>
            </SectionCard>
        ...
        <ScrollReveal delay={300}>
          <div className="flex justify-end pt-2 pb-8">
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="flex items-center gap-2 bg-[#dc2626] hover:bg-[#b91c1c] disabled:opacity-70 text-white px-7 py-3 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
            >
              <Save size={18} />
