...
export default function AdminDeals() {
  ...
  const defaultDeal = {
    title: '', sub: '', badge: '', tag: '', img: '', grad: 'from-[#dc2626] to-[#b91c1c]', bg: '#FFF4ED', border: '#e6c8a8', save: ''
  };
    ...
    <>
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/30">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage scrolling flash deals</p>
        </div>
              ...
              <tr key={d.id} className="hover:bg-red-50/30 transition-colors group">
                <td className="p-4 pl-6 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {d.save && <span className="text-xs text-red-500 font-bold">{d.save}</span>}
                  </div>
                </td>
                ...
                <td className="p-4">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {d.tag && <span className="bg-red-100 text-red-700 rounded-lg px-2.5 py-1 text-xs font-bold">{d.tag}</span>}
                  </div>
                </td>
                <td className="p-4 pr-6">
                  <div className="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button title="Edit Deal" onClick={() => { setEditing(d); setIsModalOpen(true); }} className="p-2 text-red-500 hover:bg-red-100 rounded-xl transition-colors">
                      <Edit2 size={18} />
                    </button>
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="url" placeholder="Paste URL here..." value={editing.img || ''} onChange={e => setEditing({...editing, img: e.target.value})} className="flex-1 p-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input required type="text" placeholder="e.g. UP TO 30% OFF" value={editing.title || ''} onChange={e => setEditing({...editing, title: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Subtitle <span className="text-red-500">*</span></label>
                    <input required type="text" placeholder="e.g. DRY FRUITS SALE" value={editing.sub || ''} onChange={e => setEditing({...editing, sub: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Badge</label>
                    <input type="text" placeholder="e.g. 🐕 Dogs" value={editing.badge || ''} onChange={e => setEditing({...editing, badge: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Highlight Tag</label>
                    <input type="text" placeholder="e.g. Bestseller" value={editing.tag || ''} onChange={e => setEditing({...editing, tag: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-bold text-gray-700 mb-1">Save Text</label>
                    <input type="text" placeholder="e.g. Save ₹450" value={editing.save || ''} onChange={e => setEditing({...editing, save: e.target.value})} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm" />
                  </div>

                  {/* Styling Section */}
                  <div className="md:col-span-2 bg-red-50/50 p-4 rounded-2xl border border-red-100">
                    <h4 className="font-bold text-red-800 mb-3 flex items-center gap-2">🎨 Visual Styling</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input required type="text" placeholder="from-[#dc2626] to-[#b91c1c]" value={editing.grad || ''} onChange={e => setEditing({...editing, grad: e.target.value})} className="w-full p-2 bg-white border border-gray-200 rounded-lg font-mono text-xs focus:ring-2 focus:ring-red-500/20" />
                      </div>
                      <div>
